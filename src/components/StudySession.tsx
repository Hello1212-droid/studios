...
import {
  Play, Pause, RotateCcw, Settings, X, Bell, Coffee, Brain, Zap,
  ChevronUp, ChevronDown, Lock, Unlock, Shield
} from 'lucide-react';
...
const MODE_CONFIG = {
  work: { label: 'Study Session', color: '#007AFF', glow: 'rgba(0,122,255,0.4)', icon: Brain, accent: 'blue' },
...
const StudySession: React.FC = () => {
  ...
  const enterKiosk = useCallback(() => {
    if (isElectron) {
      try {
        ...
        console.log('[StudyOS] Kiosk mode requested');
        setKioskActive(true);
      } catch (e) {
  ...
  const handleComplete = useCallback(() => {
    setIsRunning(false);
    playSound('complete');
    // When a work session completes, exit kiosk for break
    if (mode === 'work') {
      exitKiosk();
      setSessions(s => s + 1);
      showNotification('🎯 Study session complete! Take a break.');
    } else {
      showNotification('⚡ Break over! Click start to resume.');
    }
  }, [mode, playSound, showNotification, exitKiosk]);
  ...
  const switchMode = (newMode: Mode) => {
    // If switching away from work while running, exit kiosk
    if (isRunning && mode === 'work' && newMode !== 'work') {
      exitKiosk();
    }
    setMode(newMode);
    setIsRunning(false);
  ...
  const handleStart = () => {
    ...
    // When starting a work session, activate kiosk
    if (mode === 'work' && !kioskActive) {
      enterKiosk();
    }
    setIsRunning(true);
  };
  ...
  const handlePause = () => {
    setIsRunning(false);
    // Don't exit kiosk on pause — user is still in study mode
  };

  const reset = () => {
    setIsRunning(false);
    if (kioskActive && mode === 'work') {
      exitKiosk();
    }
  ...
  return (
    <div className="flex flex-col h-full bg-[#0c0c10] rounded-b-2xl overflow-hidden relative">
      ...
      <AnimatePresence>
        {kioskActive && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-3 left-3 z-50 flex items-center gap-2 px-3 py-1.5 rounded-xl bg-red-500/20 border border-red-500/30"
          >
            <Lock size={12} className="text-red-400" />
            <span className="text-red-400 text-[10px] font-black uppercase tracking-wider">Kiosk Locked</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {showSettings && (
          ...
          >
            ...
            <div className="flex items-start gap-8">
              <NumberInput value={tempSettings.work} onChange={v => setTempSettings(p => ({ ...p, work: v }))} label="Study" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 p-6">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
            ...
            <div className="text-white/40 text-xs font-medium">
              {isRunning
                ? mode === 'work' ? `Locked in — ${Math.round(progress * 100)}% remaining` : `${Math.round(progress * 100)}% remaining`
        ...
        <div className="flex items-center gap-4">
          ...
          <motion.button
            ...
            style={{
              ...
              boxShadow: isRunning
                ? `0 0 30px ${modeConfig.glow}, 0 0 60px ${modeConfig.glow}40`
                : `0 8px 32px ${modeConfig.glow}`,
            }}
          >
            {isRunning
              ? <Pause size={28} />
        ...
        {/* Kiosk Button — manual toggle */}
        {!isRunning && (
          <button
            onClick={kioskActive ? exitKiosk : enterKiosk}
          ...
          >
            {kioskActive ? (
              <><Unlock size={14} /> Exit Lockdown</>
            ) : (
              <><Shield size={14} /> Lockdown Mode</>
            )}
          </button>
        )}
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-xs">
          {[
            { label: 'Study', value: `${settings.work}m`, color: MODE_CONFIG.work.color },
